...
| `use <module>`      | войти в модуль (фокус ввода)                |
| `<module> <cmd>`    | выполнить команду модуля из любого места    |
| `<module> help`     | справка по модулю без входа в него          |
| `back`              | выйти из модуля                             |
| `clear`             | сбросить всё состояние + очистить экран     |
| `help`              | справка по модулю                           |
| `exit`              | выход                                       |

Модули не «включаются» по одному — они все активны всегда, их состояния
сосуществуют. `use` лишь выбирает, какому модулю идут короткие команды. Можно не
переключаться и звать любой модуль напрямую — несколько за раз:

```
anon tor on             # включить Tor
anon browser firefox    # закрепить браузер
gateway start           # поднять локальный сервер
# теперь Tor + браузер + шлюз работают одновременно
```
